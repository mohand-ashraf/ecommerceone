    ...
    return (
        <div className={`${theme === "dark" ? "dark" : ""}`}>
            <div className='bg-white dark:bg-bgall'>
                <div className='container xs:px-3 lg:px-3 sml:px-3 sm:px-3 md:px-3 mdl:px-3 xl:px-3 lxl:px-0 lgl:px-3 py-10 mx-auto xs:pt-[210px] sm:pt-[210px] sml:pt-[210px] md:pt-[160px] mdl:pt-[160px] lg:pt-[160px] xl:pt-[160px] lxl:pt-[160px] lgl:pt-[160px] pt-[160px]'>
                    <span className='font-Playwrite text-[25px] border-b-2 border-[#004D40] dark:border-color1 dark:text-color1 font-semibold text-[#004D40]'>Laptops</span>
                    <div className='gap-10 xl:gap-10 lxl:gap-10 lgl:gap-2 md:gap-10 justify-center flex flex-wrap items-center mt-7'>
                        {products.map((laptop) => (
                            laptop.type === 'laptop' ? (
                                <>
                                    <div key={laptop.id} className='border-blue-200 dark:border-bghead lg:mx-[13px] mdl:mx-0 lgl:mb-[30px] xl:mb-0 lxl:mb-0 xl:mx-[13px] lxl:mx-[13px] shadow-md transition-all duration-300 hover:shadow-[0_0_20px_5px_rgba(173,216,230,0.5)] border-2 rounded-2xl w-[320px] h-[550px] lgl:mx-0 mx-[13px]'>
                                        <Link to={`/product/${laptop.id}`} className='bg-white ml-2 rounded-2xl flex w-[300px] h-[360px] justify-center items-center mt-2 cursor-pointer'>
                                            <img src={`/${laptop.image}`} alt={laptop.title} className='object-contain max-w-[270px] max-h-[350px]' />
                                        </Link>
                                        <div>
                                            <Link to={`/product/${laptop.id}`}>
                                                <p className='text-[#4F5866] dark:text-[#B8BABE] px-2 text-[18px] mt-4 font-medium truncate cursor-pointer'>{laptop.title}</p>
                                            </Link>
                                            <p className='text-[#374a68] dark:text-[#B8BABE] px-2 text-[15px] mt-3 font-medium'>EGP {laptop.price}</p>
                                            <div className='gap-4 mt-7 flex items-center mx-4'>
                                                {!isProductIncart(laptop.id) ?
                                                    <button
                                                    ...
                                                    <button onClick={() => {
                                                        ...
                                                        dispatch(removeProduct(laptop.id))
                                                    }}
                                                ...
                                                <button className='transition-transform duration-300 hover:scale-110'
                                                    onClick={() => {
                                                        handleIconChange(laptop.id)
                                                    }}
                                                >
                                                    {isProductInWishList(laptop.id)
                                                        ? <FaHeart onClick={() => {
